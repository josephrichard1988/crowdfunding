...
type Campaign struct {
	...
	// Update History - tracks ALL field updates before validation (linked by CampaignID)
...
// All updates are recorded in ledger for full audit trail
type CampaignUpdate struct {
	...
	UpdatedAt    string `json:"updatedAt"`
	UpdatedBy    string `json:"updatedBy"` // StartupID who made the update
}
...
func (s *StartupContract) CreateCampaign(
	...
	campaign := Campaign{
		...
		UpdateHistory:       []CampaignUpdate{}, // Empty initially, populated by UpdateCampaign
		Status:              "DRAFT",
...
// UpdateCampaign allows startup to update campaign fields BEFORE submitting for validation
// All changes are recorded in ledger history with old value, new value, and reason
// Step 1.5: Optional - Update campaign if errors found before validation
// Channel: startup-validator-channel
// Endorsers: StartupOrg, ValidatorOrg
// Allowed only when ValidationStatus is DRAFT (before submission)
func (s *StartupContract) UpdateCampaign(
	ctx contractapi.TransactionContextInterface,
	campaignID string,
	fieldName string,    // Field to update: title, description, goalAmount, endDate, category, tags, milestones
	newValue string,     // New value for the field
	updateReason string, // Explanation for the update
) (string, error) {
	// Retrieve campaign
	...
	// Only allow updates when status is DRAFT (before submitting for validation)
	if campaign.ValidationStatus != "DRAFT" {
		return "", fmt.Errorf("campaign can only be updated when ValidationStatus is DRAFT. Current status: %s. If approved, updates are not allowed", campaign.ValidationStatus)
	}
	...
	case "title", "projectName":
		oldValue = campaign.ProjectName
		campaign.ProjectName = newValue
		updateApplied = true

	case "description":
		oldValue = campaign.Description
		campaign.Description = newValue
		updateApplied = true

	case "goalAmount":
		oldValue = fmt.Sprintf("%.2f", campaign.GoalAmount)
		var newGoalAmount float64
		if _, err := fmt.Sscanf(newValue, "%f", &newGoalAmount); err != nil {
			return "", fmt.Errorf("invalid goalAmount value: %s", newValue)
		}
		if newGoalAmount <= 0 {
			return "", fmt.Errorf("goalAmount must be greater than 0")
		}
		campaign.GoalAmount = newGoalAmount
		updateApplied = true

	case "endDate", "closeDate":
		oldValue = campaign.CloseDate
		campaign.CloseDate = newValue
		updateApplied = true

	case "startDate", "openDate":
		oldValue = campaign.OpenDate
		campaign.OpenDate = newValue
		updateApplied = true

	case "category":
		oldValue = campaign.Category
		campaign.Category = newValue
		updateApplied = true

	case "productStage":
		oldValue = campaign.ProductStage
		campaign.ProductStage = newValue
		updateApplied = true

	case "projectType":
		oldValue = campaign.ProjectType
		campaign.ProjectType = newValue
		updateApplied = true

	case "currency":
		oldValue = campaign.Currency
		campaign.Currency = newValue
		updateApplied = true

	case "durationDays":
		oldValue = fmt.Sprintf("%d", campaign.DurationDays)
		var newDuration int
		if _, err := fmt.Sscanf(newValue, "%d", &newDuration); err != nil {
			return "", fmt.Errorf("invalid durationDays value: %s", newValue)
		}
		campaign.DurationDays = newDuration
		updateApplied = true

	case "tags":
		// Tags should be JSON array: ["tag1", "tag2", "tag3"]
		oldTagsJSON, _ := json.Marshal(campaign.Tags)
		oldValue = string(oldTagsJSON)
		var newTags []string
		if err := json.Unmarshal([]byte(newValue), &newTags); err != nil {
			return "", fmt.Errorf("invalid tags format. Expected JSON array: %v", err)
		}
		campaign.Tags = newTags
		updateApplied = true

	case "fundingGoalCategory":
		oldValue = campaign.FundingGoalCategory
		campaign.FundingGoalCategory = newValue
		updateApplied = true

	case "milestones":
		// Milestones should be JSON array
		oldMilestonesJSON, _ := json.Marshal(campaign.Milestones)
		oldValue = string(oldMilestonesJSON)
		var newMilestones []Milestone
		if err := json.Unmarshal([]byte(newValue), &newMilestones); err != nil {
			return "", fmt.Errorf("invalid milestones format. Expected JSON array: %v", err)
		}
		campaign.Milestones = newMilestones
		updateApplied = true

	default:
		return "", fmt.Errorf("field '%s' is not updateable. Allowed fields: title, description, goalAmount, endDate, startDate, category, productStage, projectType, currency, durationDays, tags, fundingGoalCategory, milestones", fieldName)
	}
	...
	updateNum := len(campaign.UpdateHistory) + 1
	updateRecord := CampaignUpdate{
		UpdateID:     fmt.Sprintf("UPD_%s_%d", campaignID, updateNum),
		FieldName:    fieldName,
		OldValue:     oldValue,
		NewValue:     newValue,
		UpdateReason: updateReason,
		UpdatedAt:    now,
		UpdatedBy:    campaign.StartupID,
	}

	// Add to update history
	campaign.UpdateHistory = append(campaign.UpdateHistory, updateRecord)

	// Update timestamp
	campaign.UpdatedAt = now

	// Regenerate validation hash (since content changed)
	campaign.ValidationHash = generateCampaignHash(campaign)
	...
	eventJSON, _ := json.Marshal(eventPayload)
	ctx.GetStub().SetEvent("CampaignUpdated", eventJSON)

	response := map[string]interface{}{
		"message":        "Campaign updated successfully. All changes recorded in ledger.",
		"campaignId":     campaignID,
		"updateId":       updateRecord.UpdateID,
		...
		"reason":         updateReason,
		"validationHash": campaign.ValidationHash,
		"totalUpdates":   len(campaign.UpdateHistory),
		"status":         campaign.ValidationStatus,
		"nextStep":       "Call SubmitForValidation when ready, or UpdateCampaign for more changes",
	}
	responseJSON, _ := json.Marshal(response)
	return string(responseJSON), nil
}

// GetCampaignUpdateHistory returns the complete update history for a campaign
// Channel: startup-validator-channel
func (s *StartupContract) GetCampaignUpdateHistory(
	ctx contractapi.TransactionContextInterface,
	campaignID string,
) (string, error) {
	campaignJSON, err := ctx.GetStub().GetState(campaignID)
	...
	response := map[string]interface{}{
		"campaignId":    campaignID,
		"totalUpdates":  len(campaign.UpdateHistory),
		"updateHistory": campaign.UpdateHistory,
		"currentStatus": campaign.ValidationStatus,
	}
	responseJSON, _ := json.Marshal(response)
	return string(responseJSON), nil
}
...
func (s *StartupContract) GetProposal(ctx contractapi.TransactionContextInterface, proposalID string) (string, error) {
...
// GetProposalsByStartup retrieves all proposals for a startup
// Channel: startup-investor-channel
func (s *StartupContract) GetProposalsByStartup(ctx contractapi.TransactionContextInterface, startupID string) (string, error) {
	queryString := fmt.Sprintf(`{"selector":{"startupId":"%s","proposalId":{"$exists":true}}}`, startupID)
...
// Channel: startup-investor-channel
func (s *StartupContract) GetProposalsByCampaign(ctx contractapi.TransactionContextInterface, campaignID string) (string, error) {
