...
func (p *PlatformContract) PublishCampaignToPortal(
	...
	validationHash string, // Hash from StartupOrg to verify with Validator
	...
	// Create published campaign (initially pending verification)
	campaign := PublishedCampaign{
		...
		ValidationVerified: false, // Will be verified with Validator
	...
	eventPayload := map[string]interface{}{
		...
		"status":          "PENDING_VERIFICATION",
		"channel":         "common-channel",
		"action":          "CAMPAIGN_RECEIVED",
		"timestamp":       now,
	}
	eventJSON, _ := json.Marshal(eventPayload)
	ctx.GetStub().SetEvent("CampaignReceived", eventJSON)

	response := map[string]interface{}{
		"message":        "Campaign received. Verify with ValidatorOrg before publishing.",
		"campaignId":     campaignID,
		"validationHash": validationHash,
		"status":         "PENDING_VERIFICATION",
		"nextStep":       "Call VerifyAndPublish to verify hash with ValidatorOrg and publish",
	}
